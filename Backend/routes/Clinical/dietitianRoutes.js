/**
 * dietitianRoutes.js — Dietician workspace.
 *
...
router.get   ("/templates/:id",    validateObjectIdParam("id"), requireAction("diet.read"),  ctrl.getTemplate);
router.post  ("/templates",        requireAction("diet.write"), ctrl.createTemplate);
router.put   ("/templates/:id",    validateObjectIdParam("id"), requireAction("diet.write"), ctrl.updateTemplate);
router.delete("/templates/:id",    validateObjectIdParam("id"), requireAction("diet.write"), ctrl.deleteTemplate);
...
router.get   ("/plan/:id",            validateObjectIdParam("id"), requireAction("diet.read"),  ctrl.getPlan);
router.post  ("/plan",                requireAction("diet.write"), ctrl.createPlan);
router.put   ("/plan/:id",            validateObjectIdParam("id"), requireAction("diet.write"), ctrl.updatePlan);
// R7bb-FIX-E-9 / D6-CRIT-6: dietitian → kitchen indent push.
router.post  ("/plan/:id/kitchen-indent",
  validateObjectIdParam("id"), requireAction("diet.write"), ctrl.pushKitchenIndent);
