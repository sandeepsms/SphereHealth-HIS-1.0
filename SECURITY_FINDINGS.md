...
| ID | Title | Severity | Files | Status | Fixed-on | Verifier |
| -- | ----- | -------- | ----- | ------ | -------- | -------- |
| D-01 | 35+ `.catch(() => {})` swallow billing/clinical errors | HIGH | utils/logErr.js (new), investigationOrderController.js, medReconciliationController.js, marController.js, autoBillingService.js, admissionService.js, DoctorNotesController.js, nurseNotesController.js | PARTIAL | 2026-05-17 (r17+r18) | `logErr(module, action)` helper. Applied to ~17 sites across 7 files (investigation-order ×3, med-recon ×7, MAR auto-bill, autoBilling skip-marker, admission/discharge/transfer bed-rollback ×4, doctor + nurse note auto-bill ×2). ~18 lower-volume sites remain on backlog |
...
| E-05 | useEffect async-IIFE without AbortController | MEDIUM | pages/RoleDashboardPage.jsx | PARTIAL | 2026-05-17 (r16) | AbortController + signal-checked setState applied to AdminDashboard + DoctorDashboard effects; pattern documented for remaining sites (Nurse/Reception dashboards, PharmacyHomePage, DoctorPatientPanel) which stay on backlog |
