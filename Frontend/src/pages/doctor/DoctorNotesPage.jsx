...
// R7hr — the single-note print renders the SAME shared builder + prose mode the
// Complete IPD File uses, so an individual doctor note prints identically.
import { buildDoctorNoteCardHtml } from "./buildDoctorNoteCardHtml";
...
const MODULES = [
  ...
  // open a non-functional editor. Legacy operative notes still render via the
  // shared buildDoctorNoteCardHtml() like every other type (R7hr).
];
...
function DoctorNotesContent({ selectedPatient }) {
  ...
  const printNote = async (note) => {
    ...
    // R7hr — body from the SHARED buildDoctorNoteCardHtml() in prose mode: the
    // exact same call the Complete IPD File's Narrative EmbeddedNoteCard makes,
    // so an individual note prints byte-identical to the full file. The type
    // title + structured body + signed line come from the builder; the print
    // adds only a status strip + late-entry banner; the shell adds the
    // letterhead, patient strip and the formal Consultant/Resident sign-off.
    const noteBodyHtml = `
