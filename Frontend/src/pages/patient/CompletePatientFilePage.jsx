      ...
      <div className="pf-container">
        <IdentityBanner
          ...
          {...(() => {
            ...
            /* R7hr(DEFER-13): this was a ~250-line COPY of buildPrintReceipt
               (the mirror view's builder) and it had drifted — it still used
               the pre-R7gu-FIX iaNurseNote precedence (doctor note first), so
               the PRINT path could stamp the doctor's emp-id on the nurse IA
               signature pill while the on-screen mirror showed the right one.
               One builder now feeds both surfaces; only the not-loaded guard
               stays here (the toolbar needs a clean toast-able error). */
            const buildReceipt = () => {
              if (!data) throw new Error("Patient file not loaded yet");
              return buildPrintReceipt();
            };
            const fireFallback = () => {
