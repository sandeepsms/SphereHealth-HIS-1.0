...
function NursingNotesContent({ selectedPatient }) {
  ...
  /* ── Initial Assessment Gate (NABH COP.2) ──
     Nurse cannot write any note other than "initial" until the
     nursing initial assessment is completed and marked on the admission.
  ── */
  const nurseAssessmentDone = patient?.initialAssessment?.nurseCompleted === true;
                ...
                return (
                  <button key={m.id} onClick={() => openModal(m.id)}
                    ...
                    style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 14px", borderRadius: 9,
                      border: `1.5px solid ${isInitial && gateActive ? "#fca5a5" : locked ? C.border : m.border}`,
                      fontFamily: "'DM Sans',sans-serif", fontSize: 12, fontWeight: 600, cursor: locked ? "not-allowed" : "pointer",
                      background: isInitial && gateActive ? "#fee2e2" : "white",
                      color: locked && !isInitial ? C.muted : isInitial && gateActive ? "#dc2626" : m.color,
                      opacity: locked && !isInitial ? 0.5 : 1, transition: "all .2s", position: "relative" }}
                    onMouseEnter={e => { if (!locked) { e.currentTarget.style.background = isInitial && gateActive ? "#fecaca" : m.bg; e.currentTarget.style.transform = "translateY(-1px)"; } }}
                    onMouseLeave={e => { if (!locked) { e.currentTarget.style.background = isInitial && gateActive ? "#fee2e2" : "white"; e.currentTarget.style.transform = "none"; } }}>
                    <i className={`pi ${locked && !isInitial ? "pi-lock" : m.icon}`} style={{ fontSize: 13 }} />
                    {m.label}
                    {isInitial && gateActive && <span style={{ marginLeft: 4, background: "#dc2626", color: "white", fontSize: 9, fontWeight: 800, padding: "1px 6px", borderRadius: 3 }}>REQUIRED</span>}
                    {isInitial && !gateActive && <span style={{ marginLeft: 4, background: "#16a34a", color: "white", fontSize: 9, fontWeight: 800, padding: "1px 6px", borderRadius: 3 }}>✓ DONE</span>}
                    {m.dot && !gateActive && <span style={{ position: "absolute", top: -4, right: -4, width: 8, height: 8, background: C.red, borderRadius: "50%", border: "2px solid white" }} />}
                  </button>
                );
              })}
            </div>
