...
| D2 | **In-container automated backup broken** — nightly cron shells out to `mongodump` absent from the Alpine image (nightly ENOENT); tool-free engine wired only to a Windows task. Fresh hospital has no working backup, no alert. | **CRITICAL** (DR) | Deployment/Ops | OPEN — go-live blocker |
...
**Sprint 0 — go-live blockers (before scaling past early adopters):**
1. **D2** — bake `mongo-tools` into the image *or* schedule the tool-free backup engine from
   `provision-hospital.sh`/compose; add a backup-failure alert (the single highest-value fix).
2. **D3** — accept the free-text witness name (or resolve name→User) so HAM charting succeeds
